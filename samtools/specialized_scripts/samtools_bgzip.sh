...
echo "cd $VCF_DIR; bcftools index -t "$OUT_DIR"$VCF" > swarm_bgzip.txt #This swarm command is for indexing only
more swarm_bgzip.txt
#
read -sp "`echo -e 'Press any key to continue or Ctrl+C to abort \n\b'`" -n1 key
echo "Swarm JobID:"
#
swarm -f swarm_bgzip.txt --module samtools --time 3:00:00 --logdir ~/job_outputs/samtools --sbatch "--partition=quick --mail-type=ALL,TIME_LIMIT_80"
