...
cd $pwd
echo "cd $VCF_DIR; java -Xmx8g -jar \$PICARDJARPATH/picard.jar GatherVcfs "${sortedvcf[*]/#/$PREFIX}" O="$VCF_LOC""$FILENAME".chrAll.vcf.gz R=/data/Ostrander/Resources/cf31PMc.fa TMP_DIR=/lscratch/\$SLURM_JOB_ID" > swarm_gathervcfs.txt
#
more swarm_gathervcfs.txt
read -sp "`echo -e 'Press any key to continue or Ctrl+C to abort \n\b'`" -n1 key
echo "Swarm JobID:"
#
swarm -f swarm_gathervcfs.txt -g 10 --time 4:00:00 --module picard/2.17.11 --logdir ~/job_outputs/picard/gathervcfs --sbatch "--mail-type=ALL,TIME_LIMIT_80 --job-name $SWARM_NAME"
