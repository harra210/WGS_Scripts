...
echo "How long do you want to run your swarm?"
read -e -p "Time (in days, up to 10): " TIME
...
secs=$((TIME * 86400))
echo $secs
((sec=secs%60, secs/=60, min=secs%60, hrs=secs/60))
timestamp=$(printf "%d:%02d:%02d" $hrs $min $sec)
echo $timestamp
#swarmtime=$(printf '%dh:%dm:%ds' $(($secs/3600)) $(($secs%3600/60)) $(($secs%60)))
#echo $swarmtime
read -sp "`echo -e 'Debug mode! Press enter to continue or Ctrl+C to abort'`"
#722 Master section
...
for i in {1..38}
do
	echo "cd $GVCF_DIR; gatk --java-options \"-Xmx64g -XX:ParallelGCThreads=8\" CombineGVCFs -R /data/Ostrander/Resources/cf31PMc.fa "${list[*]/#/$PREFIX}" -D /data/Ostrander/Resources/CFA31_151.dbSNP_num_order.vcf.gz -L chr$i -O "$OUT_DIR""$OUT_NAME".chr"$i".vcf.gz --tmp-dir /lscratch/\$SLURM_JOB_ID" >> swarm_combinegvcfs.txt
#ChrX
echo "cd $GVCF_DIR; gatk --java-options \"-Xmx64g -XX:ParallelGCThreads=8\" CombineGVCFs -R /data/Ostrander/Resources/cf31PMc.fa "${list[*]/#/$PREFIX}" -D /data/Ostrander/Resources/CFA31_151.dbSNP_num_order.vcf.gz -L chrX -O "$OUT_DIR""$OUT_NAME".chrX.vcf.gz --tmp-dir /lscratch/\$SLURM_JOB_ID" >> swarm_combinegvcfs.txt
#
...
echo "Swarm JobID:"
#
swarm -f swarm_combinegvcfs.txt -g 72 -t 10 --time 240:00:00 --module GATK/4.1.0.0 --gres=lscratch:400 --logdir ~/job_outputs/gatk/combinegvcfs/ --sbatch "--mail-type=ALL,TIME_LIMIT_80 --job-name $SWARM_NAME
