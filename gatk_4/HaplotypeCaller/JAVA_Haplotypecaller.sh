...
for i in ${sample[@]}
do
	echo "cd $BQSR_DIR; gatk --java-options \"-Xmx12g\" HaplotypeCaller -R /data/Ostrander/Resources/cf31PMc.fa  -I "$i".BQSR.bam -O "$GVCF_DIR""$i"_g.vcf.gz --bam-output "$ABAM""$i"_realigned.bam --output-mode EMIT_ALL_SITES -ERC GVCF --pcr-indel-model CONSERVATIVE --smith-waterman JAVA --tmp-dir /lscratch/\$SLURM_JOB_ID -OBM -OVM" >> gatk4_HCaller_script_swarmfile.swarm
done
#NORMAL
#for ((i = 0; i < ${#directory[@]}; i++))
#do
#	echo "cd ${directory[$i]}; gatk --java-options \"-Xmx12g\" HaplotypeCaller -R /data/Ostrander/Resources/cf31PMc.fa -I "${sample[$i]}"_BQSR4.bam -O "${sample[$i]}"_g.vcf.gz --bam-output "${sample[$i]}"_realigned.bam --output-mode EMIT_ALL_SITES -ERC GVCF --pcr-indel-model NONE --smith-waterman FASTEST_AVAILABLE --TMP_DIR /lscratch/\$SLURM_JOB_ID -OBM -OVM" >> gatk4_HCaller_script_swarmfile.swarm
#done
