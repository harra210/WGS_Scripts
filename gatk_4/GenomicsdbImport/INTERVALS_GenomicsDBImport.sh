...
for i in  {1..38}
do
echo "cd $PWD; gatk --java-options \"-Xmx32g -Xms32g -DGATK_STACKTRACE_ON_USER_EXCEPTION=true\" GenomicsDBImport --genomicsdb-workspace-path "$DB_DIR"$i --batch-size 25 -L /data/Ostrander/Alex/Intervals/CanFam31_chr"$i".interval_list --max-num-intervals-to-import-in-parallel 4 --sample-name-map GenomicsDB_samplemap.txt --tmp-dir=/lscratch/\$SLURM_JOB_ID -R /data/Ostrander/Resources/cf31PMc.fa" >> GenomicsDBImport_swarmfile.swarm
done
#echo "cd $PWD; gatk --java-options \"-Xmx64g -Xms64g -DGATK_STACKTRACE_ON_USER_EXCEPTION=true\" GenomicsDBImport --genomicsdb-workspace-path "$DB_DIR"db --batch-size 25 -L /data/Ostrander/Alex/CanFam31_chr38.interval_list --max-num-intervals-to-import-in-parallel 4 --sample-name-map GenomicsDB_samplemap.txt --tmp-dir=/lscratch/\$SLURM_JOB_ID -R /data/Ostrander/Resources/cf31PMc.fa" >> GenomicsDBImport_swarmfile.swarm # SPECIALIZED
#
echo "cd $PWD; gatk --java-options \"-Xmx32g -Xms32g -DGATK_STACKTRACE_ON_USER_EXCEPTION=true\" GenomicsDBImport --genomicsdb-workspace-path "$DB_DIR"chrX --batch-size 25 -L /data/Ostrander/Alex/CanFam31_chrX.interval_list --max-num-intervals-to-import-in-parallel 4 --sample-name-map GenomicsDB_samplemap.txt --tmp-dir=/lscratch/\$SLURM_JOB_ID -R /data/Ostrander/Resources/cf31PMc.fa --verbosity DEBUG" >> GenomicsDBImport_swarmfile.swarm
#
...
#
swarm -f GenomicsDBImport_swarmfile.swarm -g 72 -t 5 --gres=lscratch:400 --module GATK/4.0.12.0 --time 240:00:00 --logdir ~/job_outputs/gatk/GenomicsDBImport --sbatch "--mail-type=ALL --job-name $SWARM_NAME"
#
