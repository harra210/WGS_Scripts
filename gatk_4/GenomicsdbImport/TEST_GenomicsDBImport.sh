...
#echo "cd $PWD; gatk --java-options \"-Xmx24g -Xms24g -DGATK_STACKTRACE_ON_USER_EXCEPTION=true\" GenomicsDBImport --genomicsdb-workspace-path "$DB_DIR"db --batch-size 150 --consolidate -L /data/Ostrander/Alex/CanFam31_Intervals.interval_list  --sample-name-map GenomicsDB_samplemap.txt --tmp-dir=/lscratch/\$SLURM_JOB_ID -R /data/Ostrander/Resources/cf31PMc.fa" >> GenomicsDBImport_swarmfile.swarm # SPECIALIZED
#
echo "cd $PWD; gatk --java-options \"-Xmx32g -Xms32g -DGATK_STACKTRACE_ON_USER_EXCEPTION=true\" GenomicsDBImport --genomicsdb-workspace-path "$DB_DIR"chrX --batch-size 25 -L chrX --sample-name-map GenomicsDB_samplemap.txt --tmp-dir=/lscratch/\$SLURM_JOB_ID -R /data/Ostrander/Resources/cf31PMc.fa --verbosity DEBUG" >> GenomicsDBImport_swarmfile.swarm
#
...
#
swarm -f GenomicsDBImport_swarmfile.swarm -g 64 -t 4 --gres=lscratch:400 --module GATK/4.0.12.0 --time 240:00:00 --logdir /data/harrisac2/job_outputs/large/gatk/GenomicsDBImport --sbatch "--mail-type=ALL --job-name $SWARM_NAME"
#
